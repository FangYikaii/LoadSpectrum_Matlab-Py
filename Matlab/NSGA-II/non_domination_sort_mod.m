...
%% function f = non_domination_sort_mod(x, M, V)
% This function sort the current popultion based on non-domination【无支配】. All the
% individuals【个体】 in the first front are given a rank of 1【给了一个1的排名】,
% the second front individuals are assigned rank 2 and so on. After assigning the rank the
% crowding in each front is calculated. 【拥挤程度】

%  Copyright (c) 2009, Aravind Seshadri
%  All rights reserved.

N = size(x,1); 
x_temp = zeros(N,M + V + 1); 
x_temp(:,1:M + V) = x(:,1:M + V); 
